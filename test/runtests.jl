...
function partition(L, ::Val{N}) where {N}
    ...
    R = [ifelse(i == 1, 1, fr[i] + 1):fr[i+1] for i = 1:N]
    ntuple(i -> R[i], N)
end


function sim(params, ::Val{Nl}, reps) where {Nl}
    Rs = [MersenneTwister(125 + 3i) for i = 1:Threads.nthreads()]
    ...
    tasks = @sync [
        Threads.@spawn begin
            ...
            noised = [
                sign.(rand(R, Nσ) .- p_flip) .* data[i]
                for j = 1:Nnoise, i in eachindex(data)
            ]
            J = CNCE(lϕ, gϕ, data, noised)
            x = sign.(randn(R, Nμ, Nσ)) .* 0.01
...
params = SimParameters(Nσ = 50, Nμ = 20, Ndata = 2, Nnoise = 20, p_flip = 1 / 3)
